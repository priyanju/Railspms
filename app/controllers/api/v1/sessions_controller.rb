...
=end
  ...
  def forgot_pwd
     user = User.where(email_id:params[:email_id],phone_number: params[:phone_number])
        if user.present?
                PasswordMailer.change_password(user, ENV["APP_URL"]).deliver_now
                render json: {response:user, status: true, msg: "Mail sent successfully"}, status: :ok
        else
                render json: false#, status: :false
        end
  end
