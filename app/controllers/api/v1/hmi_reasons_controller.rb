module Api
 module V1
  ...
  # GET /hmi_reasons
  def index
    @hmi_reasons = HmiReason.all.order("spec_id ASC")
  ...
  def create
    ...
    if @hmi_reason.save
      render json: {response: @hmi_reason, status: true, msg: "Created Successfully"}, status: :created
    else
      render json: {response: @hmi_reason, status: false, msg: "has already taken"}, status: :ok
#      render json: @hmi_reason.errors.full_messages.to_sentence, status: :unprocessable_entity
    end
  end

  def post_hmi_reason
  	@post_reason = HmiReason.new(hmi_reason_params)
	if @post_reason.save
	  render json: {response: @post_reason, status: true, msg: "Created Successfully"}, status: :created
        else
          render json: {response: @post_reason, status: false, msg: "has already taken"}, status: :ok
	end
  end
  ...
  def update
    if @hmi_reason.update(hmi_reason_params)
      render json: {response: @hmi_reason, status: true, msg: "Updated Successfully"}, status: :ok
    else
	render json: {response: @hmi_reason, status: false, msg: "Reason name or spec_id has already taken"}, status: :ok
#      render json: {response: @hmi_reason, status: false, msg: "has already taken"}, status: :false
#      render json: @hmi_reason.errors.full_messages.to_sentence, status: :unprocessable_entity
    end
  end
  ...
  def destroy
    if @hmi_reason.destroy
      render json: {status: true, msg: "Deleted Successfully"}, status: :ok
    else
      render json: {status: false, msg: "Deletion Failed"}, status: :unprocessable_entity
    end
  end
  ...
  private
    ...
    def hmi_reason_params
      params.require(:hmi_reason).permit(:name, :image_path, :is_active, :spec_id)
    end
  end
