  ...
  module V1
    class SettingsController < ApplicationController
      ...
      def report_value
        ...
        tenant = params[:tenant_id] if params[:tenant_id].present?
        setting = Tenant.find(tenant).setting 
        data = []

        data << {value: "Shiftwise"}
        data << {value: "Operatorwise"}
        #if setting.present? && setting.date_wise.present?
		#if setting.date_wise == true
          	  #data << {value: "Datewise Utilization"}
        	#end
	#end
        #if setting.present? && setting.month_wise.present?
      ...
      def resport_split_value
        ...
        if params[:report_type].present?
           if params[:report_type] == "Operatorwise"
              setting = Tenant.find(tenant).setting
              data = []
             if setting.present? && setting.hour_wise.present?
                if setting.hour_wise == true
                    data << {value: "Hourwise"}
                end
             end
            # if setting.present? && setting.program_wise.present?
