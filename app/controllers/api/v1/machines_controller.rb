...
#       idle_report = IdleReasonActive.where(date:  date.to_time.strftime("%m-%d-%Y"), shift_no: shift, machine_name: machine)
     idle_report = HmiMachineDetail.where(date:  date, shift_no: shift, machine_id: machine)
     render json: idle_report
  #  data = CncHourReport.where(date: date, machine_id: machine, shift_no: shift)

   end
  
    def idle_report_chart1
    machine = params[:machine]#Machine.where(id: params[:machine_id]).ids
    st_time = params[:date].present? ? params[:date].split('-')[0] : (Date.today - 1).strftime('%m/%d/%Y')
    date = Date.strptime(st_time, '%m/%d/%Y')
    shift = params[:shift]#Shifttransaction.where(id:params[:shift_id]).pluck(:shift_no)
     idle_report = HmiMachineDetail.where(date:  date, shift_no: shift, machine_id: machine)
     tot_rep_time = idle_report.first.data.map{|i| i["time"]}
     group_idle = idle_report.first.data.group_by{|ki| ki["idle_reason"]}
     res_value = []
###
        response = []
###
      group_idle.each do |key, value|
       re_time = value.map{|i| i["time"]}
       float_time = (re_time.sum)/(tot_rep_time.sum).to_f
       per_value = (float_time * 100).round(2)
      res_value << {
                  reason: key,
                   value: per_value
                   }
 ...
 #                  reason: key
  #                 value: per_value
   #                ]
      ...
      res_report_reason = res_value.pluck(:reason, :value)
    render json: {reason: res_report_reason}, status: :ok#res_value#{reason: res_report_reason}
#       render json: {reason: res_value, value: response}, status: :ok
#       render json: {reason: res_value}, status: :ok
  end
...
#	render json: {reason: res_value, value: response}, status: :ok
#	render json: {reason: res_value}, status: :ok
	render json: res_value
  end


   def idle_report_pareto
    machine = params[:machine]#Machine.where(id: params[:machine_id]).ids
    st_time = params[:date].present? ? params[:date].split('-')[0] : (Date.today - 1).strftime('%m/%d/%Y')
    date = Date.strptime(st_time, '%m/%d/%Y')
    shift = params[:shift]#Shifttransaction.where(id:params[:shift_id]).pluck(:shift_no)
...
#byebug
       ...
       float_time = (re_time.sum)/(tot_rep_time.sum).to_f
       per_value = (float_time * 100).round(2)
#       mins_indle = (re_time.sum/60)
	mins_indle = (re_time.sum.to_f/60).round(2)
      res_value << {
                   reason: key,
                   value: per_value,
                   mins: mins_indle
      ...
      a = []
	b = []
      res_report_reason = res_value.pluck(:reason, :mins)
      res_report_reason.each_with_index do |ind, val|
#byebug
       cons = []
       res_report_reason.each do |kk|
#byebug
          if res_report_reason[val] == kk 
          cons << kk
          cons << "$$"
          else
          cons << kk
...
#byebug
       spl_value = cons.split("$$").first
       
       individual = spl_value.last[1] 
       sum_value = spl_value.map{|i| i[1]}.sum      
       dub_value = ((sum_value).to_f/(tot_rep_time.sum/60).to_f).round(2)*100
       
      #coll_value  << {reason: ind[0], cumulative: dub_value, individual: individual}
	a << ind[0]
	coll_value << dub_value
	b << individual
	###vino
	mine = spl_value
end
#     render json: coll_ivalue  
	ab = [], bc = []
	descending = -1
	mine = mine.sort_by { |h| h[1] * descending }
	ab, bc = mine.transpose
#	render json: {reason: a,}
	render json: {reason: ab, individual: bc, cumulative: coll_value}, status: :ok
end
 ...
 # GET /machines/1
  def show
    ...
    if Machine.all.present? && Shifttransaction.all.present?
...
unless data1 == 0
	...
	k = "idle2" if k == 2
     ...
     ff[key1] = value
	keyset = ["idle", "idle1", "idle2"]
        remain = value.slice(*keyset)
        remain = remain.values.sum
        remainder[key1] = remain
    end
    render json: {"data" => data1.group_by{|d| d[:unit]}, count: ff, idle_count: remainder}
   ...
   def single_machine_live_status
	 ...
	 #   RestClient.post "http://13.234.15.170/api/v1/rest_machine_create", @machine.attributes, {content_type: :json, accept: :json}
	    ...
	    if @machine.save
 		...
 		render json: {response: @machine, status: true, msg: "Created Successfully"}, status: :ok
   	    else
      		render json: {response: @machine, status: false, msg: "Machine IP or Machine name has already taken"}, status: :ok
...
###########priya
	   def oee_reports
		data = HourReport.oee_reports_priya(params)
		render json: data
	   end
	 ...
	 #   RestClient.post "http://13.234.15.170/api/v1/rest_machine_update", @machine.attributes, {content_type: :json, accept: :json}
	   ...
	   # else
	    #  render json: @machine.errors, status: :unprocessable_entity
	    #end
		render json: {response: @machine, status: true, msg: "Updated Successfully"}, status: :ok
            else
                render json: {response: @machine, status: false, msg: "Machine IP or Machine name has already taken"}, status: :ok
	 ...
	 #render json: machine
	  ...
	  def alarm_api
		...
		render json: true
	       ...
	       alarm = Alarm.create(alarm_type: params[:alarm_type],alarm_number:params[:alarm_number],alarm_message: params[:alarm_message],emergency: params[:emergency],machine_id: iid)
		if alarm.save
			render json: true
		else 
	                render json: false
...
#byebug
  ...
  def hour_wise_report_data
     hour_wise_report_chart= HourReport.hour_wise_report_chart(params)
     render json: hour_wise_report_chart
  end
