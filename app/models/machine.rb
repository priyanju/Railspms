...
validates :machine_ip, presence:true
validates :machine_ip, uniqueness:true
...
has_many :machine_signals

has_one :current_part
has_many :parts
has_many :macro_signals
has_one :macro_signal_opened
...
#byebug
        	...
        	status = 0
	        collect_data << {status: status, du_time: (end_time - start_time).to_i}
           end
	 end
    else
      du_value.each_with_index do |data, index|
