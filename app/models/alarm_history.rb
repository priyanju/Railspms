...
#byebug
          if AlarmReport.where(date: data[0], shift_no: data[1], alarm_time: data[2], message:data[3], alarm_no: data[4], alarm_type: data[5], machine_id: data[7], shift_id: data[8], operator_id: data[10]).present?
            puts "Go Go"
          else
            AlarmReport.create(date: data[0], shift_no: data[1], alarm_time: data[2], message:data[3], alarm_no: data[4], alarm_type: data[5], axis_no: data[6], category: data[7], machine_id: data[7], shift_id: data[8], tenant_id: data[9], operator_id: data[10])
          end
        end
      end
end
...
def self.alarm_histories_report12(tenant, shift_no, date)
    date = date
    @alldata = []
    tenant = Tenant.find(tenant)
    machines = tenant.machines.where(controller_type: 1)
      ...
      if @alldata.present?
        @alldata.each do |data| 
          if AlarmReport.where(date: data[0], shift_no: data[1], alarm_time: data[2], message:data[3], alarm_no: data[4], alarm_type: data[5], machine_id: data[7], shift_id: data[8], operator_id: data[10]).present?
            ...
            AlarmReport.create(date: data[0], shift_no: data[1], alarm_time: data[2], message:data[3], alarm_no: data[4], alarm_type: data[5], axis_no: data[6], machine_id: data[7], shift_id: data[8], tenant_id: data[9], operator_id: data[10])
          end 
        end
      end
  end
