...
def self.all_cycle_time_chat1(params)
  pg_num_diff = [];
  machine = Machine.find(params[:machine_id]).id
  date = params[:date].to_date.strftime("%Y-%m-%d")
  shift = Shifttransaction.find_by(id:params[:shift_id]).shift_no
  data = CncReport.where(date: date, machine_id: machine, shift_no: shift)
  if data.present?
     time = data.first.parts_data
     data.first.parts_data.each do |i|
#byebug
        unless eval(i)[0][:program_number] == "" && eval(i)[0][:program_number] == 0

         all_cycle_value = eval(i)
         all_cycle_value = all_cycle_value.inject(&:merge)
         #all_cycle_value = all_cycle_value.map{|k,v| "#{k}:#{v}"}.join(',')

        shift_no = eval(i)[0][:shift_no] = data.first.shift_no

        time = eval(i)[0][:time] = data.first.time

        pg_num_diff << [i,shift_no,time,all_cycle_value]
        end
      end
...
#      eval(i)[0][:shift_no] = data.first.shift_no

 #     eval(i)[0][:time] = data.first.time
	i[:shift_no] = data.first.shift_no
        i[:time] = data.first.time
	
	pg_num_diff << i
#       data.each do |i|
#	a = i.parts_data.first
#	unless eval(a)[0][:program_number] == "" && eval(a)[0][:program_number] == 0
#	   i[:shift_no] = data.first.shift_no
#	   i[:time] = data.first.time
#           pg_num_diff << a
    end
...
#byebug
  machine = Machine.where(id: params[:machine_id]).ids
  date = params[:date].to_date.strftime("%Y-%m-%d")
  date_time = date.to_time.strftime("%I:%M")
  shift = Shifttransaction.where(id:params[:shift_id]).pluck(:shift_no)
  data = CncHourReport.where(date: date, machine_id: machine, shift_no: shift)
...
#byebug
    if pgnum.present?
#      program_number << eval(pgnum.first)[0][:program_number]
      program_number << pgnum.pluck(:program_number)
    end
  end
  time = data.pluck(:time)
...
#  data5 = CncReport.where(date: date, machine_id: machine, shift_no: shift).first
 program_number = []
  data.pluck(:all_cycle_time).each do |pgnum|
    if pgnum.present?
#	 a = pgnum[0]
#      program_number << eval(a)[0][:program_number]
      program_number << pgnum.pluck(:program_number)
    end
  end
...
#      program_number << eval(a)[0][:program_number]
        #  program_number << pgnum.pluck(:program_number)
	data.pluck(:parts_data).each do |pgnum|
	if pgnum.present?
	program_number << pgnum.pluck(:program_number)
        end
       end
...
#############
	duration1, idle1, no_data1, actual_running1  = duration.map(&:to_f), idle.map(&:to_f), no_data.map(&:to_f), actual_running.map(&:to_f)
	idle_percentage = [idle1, duration1].transpose.map {|x| x.reduce(:/)}
	idle_percent = []
	idle_percentage.each { |x| idle_percent << (x*100).round(2) }
	no_data_percentage = [no_data1, duration1].transpose.map {|x| x.reduce(:/)}
	no_data_percent = []
	no_data_percentage.each { |x| no_data_percent << (x*100).round(2) }
	actual_running_percent = []
	actual_running_percentage = [actual_running1, duration1].transpose.map {|x| x.reduce(:/)}
	actual_running_percentage.each { |x| actual_running_percent << (x*100).round(2)}
	
	return {time: time, duration: duration, run_time: actual_running_percent, idle_time: idle_percent, stop_time: stop_time, no_data: no_data_percent,shift_no: shift_number}	
      #return {time: time, duration: duration, run_time: actual_running, idle_time: idle, stop_time: stop_time, no_data: no_data,shift_no: shift_number, idle_percentage: idle_percent, no_data_percentag: no_data_percent, actual_running_percentage: actual_running_percent }
  end
...
def self.all_cycle_time_chart_new_vino(params)
 ...
 pg_num_diff = [];
  ...
  next if time == []
   ...
   #else
  #m@data = time.pluck(:cycle_time).sum/time.count
  test = []
   cy_tym  = data2.pluck(:all_cycle_time)
   cy_tym.each do |x|
     test << eval(x)[0][:cycle_time]
   end
  @data = test.sum/test.count
  if time.present?
...
def self.hour_wise_report_chart(params)
#byebug
	tenant=Tenant.find(params[:tenant_id])
 	machine = Machine.where(id: params[:machine_id]).ids
	shift = Shifttransaction.where(id: params[:shifttransaction_id])
  	data = CncHourReport.where(date: params["start_date"]..params["end_date"], machine_id: machine, shift_no: shift)
#
	if data.present?
	  return data
	else
...
def self.all_cycle_time_chart_new1(params)
 pg_num_diff = [];
  tenant=Tenant.find(params[:tenant_id])
  #shift = Shifttransaction.where(id:params[:shift_id]).pluck(:shift_no)
   if params[:shift_id] == "undefined"
  ...
  if data2.present?
  data2.each do |data|
  time = data.parts_data#.pluck(:cycle_time)
  next if time == []
     #return pg_num_diff.flatten.group_by { |d| d[:shift_no] }
   #else
   datum = time.map {|x| x["cycle_time"]}
   @data = datum.sum/time.count	
  #@data = time.pluck(:cycle_time).sum/time.count
  if time.present?
    data.parts_data.each do |ind|
    unless ind[:program_number] == "" && ind[:program_number] == 0
      ind[:shift_no] = data.shift_no
      ind[:time] = data.time
      pg_num_diff << ind.merge(average: @data)
...
#   byebug
#	pg_num_diff =  pg_num_diff.flatten.group_by { |d| d[:shift_no] }
#	pg_num_diff1 =  pg_num_diff.map {|x| x["cycle_time"]}
#byebug
  return pg_num_diff.flatten.group_by { |d| d[:shift_no] }
end
