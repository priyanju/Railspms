  ...
  def self.notification(machine_log)
	...
	machine_log = MachineLog.find(machine_log)
     ...
     if machine_log.machine.set_alarm_settings.present?
       ...
       player_id = OneSignal.where(user_id:machine_log.machine.tenant.users.ids).pluck(:player_id).uniq
  ...
  if params[:report_type] == "Shiftwise"   
   ...
   shift_no = shifts.pluck(:shift_no)
	alarm = AlarmReport.where(date: dates, shift_no: shift_no, machine_id: machine_id).order(:date)
	alarm = alarm.paginate(page: params[:page], per_page: params[:per_paage])
...
#   return AlarmReport.where(date: dates, shift_no: shift_no, machine_id: machine_id).order(:date)
	...
	alarm = AlarmReport.where(date: dates, operator_id: operator_id, machine_id: machine_id).order(:date)
        alarm = alarm.paginate(page: params[:page], per_page: params[:per_paage])
        return alarm

 #   return AlarmReport.where(date: dates, operator_id: operator_id, machine_id: machine_id).order(:date)
  end    
 end
